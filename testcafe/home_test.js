...
test('renders top playlists sidebar', async (t) => {
    ...
    await t.expect(topPlaylists.count).gte(1);
});
...
// Playlist sidebar
test('renders 10 placeholder playlists', async (t) => {
    await t.expect(topPlaylists.count).eql(10);
});

test('each playlist has an image and a name', async (t) => {
    const firstPlaylist = topPlaylists.nth(0);
    const img  = firstPlaylist.find('img');
    const name = firstPlaylist.find('span');

    await t.expect(img.exists).ok();
    await t.expect(name.innerText).contains('Playlist');
});